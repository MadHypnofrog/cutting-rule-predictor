...
"""
	...
	metrics : array-like
		Array of metrics to use in calculations. All elements should be valid metrics to use in StatsCalculator.stats_for_rules.
	stats_d : dict of pairs (metric_name, [baseline, metrics]), optional
		Statistics for the dataset (same format as in StatsCalculator.read_stats or write_stats). If present, no calculation
		is done and the statistics are just written to the HTMLBuilder.

	Returns
	-------
	dict of pairs (metric_name, [baseline, metrics]) - statistics for the dataset. If stats_d is not None, then it is returned.
"""
...
"""
	Iterates through all datasets in a specified folder, collects their statistics and writes them to a file and a HTMLBuilder.
	...
	stats : dict of pairs (ds_name, dict of pairs (metric_name, [baseline, metrics])), optional
		Precomputed stats for the datasets. Would be also saved to the log file if it is present. See StatsCalculator for dict format.
