...
class MetaModel():
	...
	"""
		...
		filepath : str, optional
			Path to the log file. If present, all output (configurations and their losses) will be logged into that file.
	"""
	...
	"""
		...
		stats : array-like, shape (n_datasets, 2)
			Cutting rule statistics for the datasets. First column consists of float values and represents baselines, the second
			column consists of lists and represents metric values for each cutting rule.
	...
	def perform_kfold(self, html_path, kfold_table_name, model, datasets, ds_names, stats, modes, splits=5):
		...
		rankings = np.array(list(map(lambda metric: np.argsort(metric)[::-1], metrics)))  # hacks because of variable length arrays
		total_losses = []
	...
	"""
		...
		Returns
		-------
		array_like, shape (n_configs) : array of pairs (KNNConfig, array of mean of losses for each mode). If self.handle is not None,
		those results are also written to the log file.
	"""
	def knn_grid(self, datasets, ds_names, stats, k_range=np.arange(2, 20, 1), exp_range=(np.arange(0.5, 1.0, 0.05)), 
		...
		for k in k_range:
			for ranking_exp in exp_range:
				losses = self.perform_kfold_loss_only(KNNModel(k=k, mode='knn', ranking_exp=ranking_exp), 
					datasets, ds_names, stats, modes, splits=splits)
				configs.append((KNNConfig(k, 'knn', 'euclidean', None, None, ranking_exp), [round(np.mean(loss), 4) for loss in losses]))
		...
		for ranking_exp in exp_range:
			for kernel in kernels:
				for window in window_range:
					losses = self.perform_kfold_loss_only(KNNModel(mode='parzen-fixed', window=window, kernel=kernel, ranking_exp=ranking_exp), 
						datasets, ds_names, stats, modes, splits=splits)
					configs.append((KNNConfig(None, 'parzen-fixed', 'euclidean', kernel, window, ranking_exp), [round(np.mean(loss), 4) for loss in losses]))
					if self.handle is not None:
		...
		for k in k_range:
			for ranking_exp in exp_range:
				for kernel in kernels:
					losses = self.perform_kfold_loss_only(KNNModel(mode='parzen-variable', k=k, kernel=kernel, ranking_exp=ranking_exp), 
						datasets, ds_names, stats, modes, splits=splits)
					configs.append((KNNConfig(k, 'parzen-variable', 'euclidean', kernel, None, ranking_exp), [round(np.mean(loss), 4) for loss in losses]))
					if self.handle is not None:
