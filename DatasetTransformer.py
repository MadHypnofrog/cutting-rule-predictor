...
class DatasetTransformer():
	...
	"""
		...
		objects_threshold : int, optional
			The minimal around of different values a feature has to have to be considered numeric. Useful on datasets with low object count.
		objects_classes : dict, optional
			A dictionary that contains pairs of (object_count, num_classes). If a dataset has more than object_count objects, any numeric
			features in it would be discretized into num_classes bins. The more objects a dataset has, the more bins it requires to maintain
			classification quality.
		objects_divisor : int, optional
			The threshold divisor. If a feature has more than num_objects / objects_divisor different values, it is considered numeric. Gets
			overrided by objects_threshold so that datasets with low object count would not treat every feature as numeric.
	"""
		...
		for col in df.columns:
			if col != 'class':
				...
				if values == 2 and df.dtypes[col] != 'object':
					...
					ohe = pd.get_dummies(pd.qcut(df[col], q=num_classes, labels=False, precision=0, duplicates='drop'), prefix=col)
					if ohe.shape[1] == 1:
						ohe = pd.get_dummies(df[col], prefix=col)
					res = pd.concat([res, ohe], axis=1)
