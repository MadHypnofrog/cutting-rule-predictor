...
header = ''.join(['<html><head><link rel=\"stylesheet\" href=\"styles.css\"></head>',
	'<body><table><tr><th>Dataset name</th><th>Baseline (micro)</th><th>Graph (micro)</th><th>Best F1 (micro)</th><th>Number of peaks/valleys (micro)</th>',
	'</tr>'])
header_kfold = ''.join(['<html><head><link rel=\"stylesheet\" href=\"styles.css\"></head>',
	'<body><table><tr><th>Dataset name</th><th>Graph</th><th>Min value / max value</th><th>Loss1</th><th>Loss2</th><th>Loss3</th></tr>'])
tr = '<tr>%s</tr>'
td = '<td>%s</td>'
...
class HTMLBuilder():
	...
	def gen_meta(self, df, ds_name):
		...
		return td % ''.join([ds_name, '<br>', num_classes, ' classes, ', num_features, ' features, ', num_objects, ' objects'])
