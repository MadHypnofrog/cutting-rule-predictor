...
class KNNModel():
    ...
    """
        ...
        mode : str, optional
            Type of algorithm to use when assigning weights to the training set. Possible values are 'knn', 
            'parzen-fixed' and 'parzen-variable'.
        metric : str or callable, optional
            Distance metric to use in pairwise distances computing. If str, should be a valid metric as specified in 
            scipy's cdist.
        kernel : str or callable, optional
            Kernel function to use when assigning weights to the training set. If str, should be a kernel specified in 
            Kernels.
    ...
    """
        ...
        Returns
        -------
        KNNConfig : configuration for this model
    """
    def get_config(self):
        return KNNConfig(self.k, self.mode, self.metric, self.kernel, self.window, self.ranking_exp)
    ...
    def prediction_mode(self, predictions, top, radius, random=False):
        ...
        for i in range(predictions.shape[0]):
            ...
            indices = ranking[:top]
            result.append(np.concatenate([[x for x in range(idx - radius, idx + radius + 1, 1) if x >= 0 and x < max_f] 
                for idx in indices]))
        return np.array(result)
